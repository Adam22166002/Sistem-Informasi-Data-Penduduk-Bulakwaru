...
@section('sub-title', 'Mengelola Data Penduduk Desa Bulakwaru')
...
@include('admin.penduduk.modal_penduduk')
@include('admin.penduduk.modal_kk')
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(`{{ asset('assets/geojson/batas-bulakwaru.geojson') }}`)
    .then(r => r.json())
    .then(json => {
    ...
    function loadRTRW() {
        fetch('{{ asset("assets/geojson/batas-rt-rw01.geojson") }}')
            .then(r => r.json())
            .then(json => {
    ...
    window.updateLocalByKK = function(pendudukList, rumahId) {
        let rumah = rumahData.find(r => r.id == rumahId);
        if (!rumah) return;

        pendudukList.forEach(p => {
            if (!rumah.penduduk) rumah.penduduk = [];
